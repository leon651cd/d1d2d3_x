...
def train_and_test(nb_epochs, weight, method, degree, random_seed,ratio):
    ...
    # Data
    trainx, trainy = data.get_train(ratio)
    trainx_copy = trainx.copy()

    testx, testy = data.get_test(ratio)
    test_label=data.get_test_label(ratio)
    test_ip=data.get_test_ip(ratio)


    trainx_a,trainy_a=data.get_train_a(ratio)
    trainx_a_copy=trainx_a.copy()
    a_label_train=data.get_a_label_train(ratio)
    ...
    with tf.name_scope('optimizers'):
        ...
        optimizer_att = tf.train.AdamOptimizer(learning_rate=0.05, beta1=0.5, name='att_optimizer')
