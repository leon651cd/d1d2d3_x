...
import train_data_bigan.data3 as data
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    with tf.variable_scope('generator_model'):
        z = tf.random_normal([batch_size, 1,times])
        x_gen = gen(z, is_training=is_training_pl)

        z_a=tf.random_normal([batch_size_a, 1,times])
    ...
    with tf.name_scope('loss_functions'):
        ...
        loss_generator = 10*tf.reduce_mean(lg1) + tf.reduce_mean(lg2)

        # encoder
        le1=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(l_encoder), logits=l_encoder)
        le2=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(l_encoder_a), logits=l_encoder_a)
        loss_encoder = tf.reduce_mean(le1)+tf.reduce_mean(le2)
    ...
    with tf.name_scope('Testing'):
        ...
        with tf.variable_scope('Score'):
            list_scores = 0.7*gen_score + 0.1* dis_score+ 0.2*dis_score_a           #0.4 0.1 0.5调整后差别不大
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            if((epoch+1)%1==0):
                ...
                fill = np.ones([batch_size - size, characters, times])
