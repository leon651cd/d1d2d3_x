...
def discriminator_1(z_inp, x_inp, is_training=False, getter=None, reuse=False):
    ...
    with tf.variable_scope('discriminator_1', reuse=reuse, custom_getter=getter):
        ...
        with tf.variable_scope("dis1_lstm1"):
            ...
            tensor_attention=tf.transpose(tensor_a,perm=[0,2,1])
            attention_o=attention(tensor_attention,25)


        with tf.variable_scope("dis1_lstm2"):
            ...
            tensor_attention_=tf.transpose(tensor_a_,perm=[0,2,1])
            attention_o_=attention(tensor_attention_,25)
            print(attention_o_.shape)


            final = tf.concat([attention_o, attention_o_], axis=1)
            print(final.shape)
            final = leakyReLu(final)
        name_y = 'y_fc_1'
...
def discriminator_2(z_inp, x_inp, is_training=False, getter=None, reuse=False):
    ...
    with tf.variable_scope('discriminator_2', reuse=reuse, custom_getter=getter):
        ...
        with tf.variable_scope("dis2_lstm2"):
            z_prior = tf.unstack(z_inp, times, 2);  #
            lstm_cell_ = tf.contrib.rnn.MultiRNNCell(
                [tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(36)) for _ in range(2)]);
            res_, states_ = tf.contrib.rnn.static_rnn(lstm_cell_, z_prior, dtype=tf.float32);
...
x = tf.constant(3.,shape=[13,56,10])
discriminator_1(z,x)