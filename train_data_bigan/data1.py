...
def _get_dataset():
    ...
    x_train = n_data.astype(np.float32)
    y_train = n_data_label.astype(np.float32)

    x_test = np.concatenate((n_data,a_data),axis=0)
    x_test = x_test.astype(np.float32)
    y_test = np.concatenate((n_data_label,a_data_label),axis=0)
